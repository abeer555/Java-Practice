...
public class O2D
{
    public static void main() {
        ...
        for(i=0,j=s.length()-1;i<s.length();i++,j--){
            da=Character.getNumericValue(s.charAt(i))*Math.pow(8,j);
            System.out.println(s.charAt(i)+" * 8"+superscript(String.valueOf(j))+"  = "+(int)da);
            b+=da;
            arr1[i]=(int)da;
        }
        int k=i;
        ...
        for(i=-1,j=0;j<decimal.length();i--,j++,k++) {
            da=Character.getNumericValue(decimal.charAt(j))*Math.pow(8,i);
            System.out.println(decimal.charAt(j)+" * 8"+superscript(String.valueOf(i))+" = "+da);
            b+=da;
            arr1[k]=da;
        }
